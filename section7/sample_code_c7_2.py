...
def execute_two_microphone_sparse_separation(x_left, x_right, is_use_amplitude=False):
    if is_use_amplitude == True:
        ...
        y_left = (amp_ratio > 1.0).astype(np.float) * x_left

        y_right = (amp_ratio <= 1.0).astype(np.float) * x_right

    else:
        # 位相差を用いた分離
        phase_difference = np.angle(x_left / x_right)

        y_left = (phase_difference > 0.0).astype(np.float) * x_left

        y_right = (phase_difference <= 0.0).astype(np.float) * x_right
...
snr_pre = calculate_snr(multi_conv_data_left_no_noise[0, ...], multi_conv_data[0, ...]) + calculate_snr(
    multi_conv_data_right_no_noise[1, ...], multi_conv_data[1, ...]
)
snr_phase_post = calculate_snr(multi_conv_data_left_no_noise[0, ...], y_phase_left) + calculate_snr(
    multi_conv_data_right_no_noise[1, ...], y_phase_right
)
...
# ファイルに書き込む
write_file_from_time_signal(y_phase_left * np.iinfo(np.int16).max / 20.0, "./near_sparse_phase_left.wav", sample_rate)
write_file_from_time_signal(y_phase_right * np.iinfo(np.int16).max / 20.0, "./near_sparse_phase_right.wav", sample_rate)
write_file_from_time_signal(y_amp_left * np.iinfo(np.int16).max / 20.0, "./near_sparse_amp_left.wav", sample_rate)
write_file_from_time_signal(y_amp_right * np.iinfo(np.int16).max / 20.0, "./near_sparse_amp_right.wav", sample_rate)
