...
def batch_kron(A, B):
    if np.shape(A)[:-2] != np.shape(B)[:-2]:
        ...
        return np.reshape(
            np.einsum("...mn,...ij->...minj", A, B), np.shape(A)[:-2] + (np.shape(A)[-2] * np.shape(B)[-2], np.shape(A)[-1] * np.shape(B)[-1])
        )
...
def execute_ip_time_varying_gaussian_ilrma_t(x, x_bar, P, a, b, n_iterations=20):
    ...
    for t in range(n_iterations):
        ...
        # 共分散行列を算出
        Q = np.einsum("skt,ktij->tksij", 1.0 / np.maximum(v, 1.0e-18), x_hat_x_hat_H)
        Q = np.average(Q, axis=0)
        ...
        for source_index in range(M):
            ...
            p = np.einsum("km,k->km", Q_inverse_b, 1.0 / np.sqrt(np.maximum(np.abs(b_h_Q_inverse_b), 1.0e-18)))
            P[:, source_index, :] = np.conjugate(p)

        # コスト計算
        cost = np.sum(np.mean(s_power / np.maximum(v, 1.0e-18) + np.log(v), axis=-1)) - np.sum(2.0 * np.log(np.abs(np.linalg.det(P[:, :, :M]))))
        cost_buff.append(cost)
        # print(t,cost)
...
def execute_ip_time_varying_gaussian_ilrma_dereverb(x, x_bar, W, a, b, n_iterations=20):
    ...
    for t in range(n_iterations):
        ...
        # 入力信号の共分散行列を求める
        V_inverse = np.einsum("skt,ksm,ksn->ktmn", 1.0 / np.maximum(v, 1.0e-18), np.conjugate(W), W)
        ...
        # IP法による更新
        Q = np.einsum("skt,mkt,nkt->tksmn", 1.0 / np.maximum(v, 1.0e-18), x_dereverb, np.conjugate(x_dereverb))
        Q = np.average(Q, axis=0)
...
# 畳み込んだ波形をファイルに書き込む
write_file_from_time_signal(multi_conv_data_left_no_noise[0, :] * np.iinfo(np.int16).max / 20.0, "./lgm_dereverb_left_clean.wav", sample_rate)

# 畳み込んだ波形をファイルに書き込む
write_file_from_time_signal(multi_conv_data_right_no_noise[0, :] * np.iinfo(np.int16).max / 20.0, "./lgm_dereverb_right_clean.wav", sample_rate)

# 畳み込んだ波形をファイルに書き込む
write_file_from_time_signal(multi_conv_data[0, :] * np.iinfo(np.int16).max / 20.0, "./lgm_dereverb_in_left.wav", sample_rate)
write_file_from_time_signal(multi_conv_data[0, :] * np.iinfo(np.int16).max / 20.0, "./lgm_dereverb_in_right.wav", sample_rate)
...
# ILRMA-T実行
Wilrma_t, s_ilrma_t, cost_buff_ilrma_t = execute_ip_time_varying_gaussian_ilrma_t(
    stft_data, x_bar, Pilrma_t, a.copy(), b.copy(), n_iterations=n_ica_iterations
)

y_ilrma_t = projection_back(s_ilrma_t, Wilrma_t)

# ILRMA+Dereverb実行 (IP法ベース)
Wilrma_dereverb_ip, s_ilrma_dereverb_ip, cost_buff_ilrma_dereverb_ip = execute_ip_time_varying_gaussian_ilrma_dereverb(
    stft_data, x_bar, Wilrma_ip.copy(), a.copy(), b.copy(), n_iterations=n_ica_iterations
)

y_ilrma_dereverb_ip = projection_back(s_ilrma_dereverb_ip, Wilrma_dereverb_ip)

# ILRMA実行 (IP法ベース)
Wilrma_ip, s_ilrma_ip, cost_buff_ilrma_ip = execute_ip_time_varying_gaussian_ilrma(
    stft_data, Wilrma_ip, a.copy(), b.copy(), n_iterations=n_ica_iterations
)
...
snr_ilrma_t_post1 = calculate_snr(multi_conv_data_left_no_noise[0, ...], y_ilrma_t[0, ...]) + calculate_snr(
    multi_conv_data_right_no_noise[0, ...], y_ilrma_t[1, ...]
)
snr_ilrma_t_post2 = calculate_snr(multi_conv_data_left_no_noise[0, ...], y_ilrma_t[1, ...]) + calculate_snr(
    multi_conv_data_right_no_noise[0, ...], y_ilrma_t[0, ...]
)

snr_ilrma_t_post = np.maximum(snr_ilrma_t_post1, snr_ilrma_t_post2)
snr_ilrma_t_post /= 2.0
...
snr_ilrma_dereverb_ip_post1 = calculate_snr(multi_conv_data_left_no_noise[0, ...], y_ilrma_dereverb_ip[0, ...]) + calculate_snr(
    multi_conv_data_right_no_noise[0, ...], y_ilrma_dereverb_ip[1, ...]
)
snr_ilrma_dereverb_ip_post2 = calculate_snr(multi_conv_data_left_no_noise[0, ...], y_ilrma_dereverb_ip[1, ...]) + calculate_snr(
    multi_conv_data_right_no_noise[0, ...], y_ilrma_dereverb_ip[0, ...]
)

snr_ilrma_dereverb_ip_post = np.maximum(snr_ilrma_dereverb_ip_post1, snr_ilrma_dereverb_ip_post2)
snr_ilrma_dereverb_ip_post /= 2.0
...
write_file_from_time_signal(y_ilrma_dereverb_ip[0, ...] * np.iinfo(np.int16).max / 20.0, "./ilrma_dereverb_ip_1.wav", sample_rate)
write_file_from_time_signal(y_ilrma_dereverb_ip[1, ...] * np.iinfo(np.int16).max / 20.0, "./ilrma_dereverb_ip_2.wav", sample_rate)

write_file_from_time_signal(y_ilrma_t[0, ...] * np.iinfo(np.int16).max / 20.0, "./ilrma_t_1.wav", sample_rate)
write_file_from_time_signal(y_ilrma_t[1, ...] * np.iinfo(np.int16).max / 20.0, "./ilrma_t_2.wav", sample_rate)
